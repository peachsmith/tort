...
static void handoff(cr_app *app, cr_func next_scene)
{
    ...
    handle = app->extension->entity_handles[TORT_HANDLE_TRANSITION];
    if (handle == NULL)
    {
        return;
    }
...
void tort_register_transition(cr_entity_type *t)
{
    t->width = 50;
    t->height = 50;
...
cr_entity *tort_create_transition(cr_app *app)
{
